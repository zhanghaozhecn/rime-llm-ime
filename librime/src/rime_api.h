...
typedef struct RIME_FLAVORED(rime_api_t) {
  ...
  //! 设置光标前上文文本 (weasel TSF 前端在按键时获取, server 调用)
  void (*set_context_text)(const char* text);
  //! 获取光标前上文文本 (lua 通过 rime_api 调用; 未设置时返回空串)
  const char* (*get_context_text)(void);
  //! 光标前上文是否已由 TSF 前端成功采集过 (True 时 get_context_text
  //! 的空串是"合法空上文"而非"采集不可用"; False 表示 TSF 采集从未成功,
