...
// 光标前上文文本缓存 (weasel TSF 前端按键时获取 → set_context_text;
// lua / llm_filter 通过 rime_api.get_context_text 读取)。
// 写入: weasel server 消息线程; 读取: 引擎线程 + llm_filter prepare 轮询线程
// (commit 后异步等待上文更新) → 需要互斥保护。
// get 返回 thread_local 缓冲, 调用方应立即拷贝, 锁外安全。
static std::mutex g_context_text_mutex;
...
static void RimeSetContextText(const char* text) {
  ...
  g_context_text = text ? text : "";
  g_context_text_valid = true;  // any arrival means TSF collection works
  if (g_context_changed_cb)
    g_context_changed_cb(g_context_text.c_str());
}
static void RimeSetContextChangedCallback(
    void (*callback)(const char* text)) {
  std::lock_guard<std::mutex> lock(g_context_text_mutex);
  g_context_changed_cb = callback;
}
static const char* RimeGetContextText() {
  std::lock_guard<std::mutex> lock(g_context_text_mutex);
  g_context_text_tls = g_context_text;
  return g_context_text_tls.c_str();
}
...
RIME_API RIME_FLAVORED(RimeApi) * RIME_FLAVORED(rime_get_api)() {
  ...
  if (!s_api.data_size) {
    ...
    s_api.get_context_text = &RimeGetContextText;
